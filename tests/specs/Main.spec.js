...
describe('<FullHeader />', () => {
  ...
  context('title', () => {
    ...
    it('should have h1 tag with the title passed', () => {
      const wrapper = shallow(<FullHeader title="TDD" />);
      expect(wrapper.find('h1').props().children).to.equal('TDD');
    });
  });
  ...
  context('textColor', () => {
    it('should have color equal #fff when none is passed', () => {
      const wrapper = shallow(<FullHeader text="TDD" />);
      expect(wrapper).to.have.style('color').equal('#fff');
    });

    it('should have color equal #ff000 when none is passed', () => {
      const wrapper = shallow(<FullHeader text="TDD" textColor="#ff0000" />);
  ...
  context('video', () => {
    ...
    it('should have video tag with the video passed', () => {
      const wrapper = shallow(<FullHeader video="my_video.mp4" />);
      expect(wrapper.find('video').props().src).to.equal('my_video.mp4');
    });
  });
});
